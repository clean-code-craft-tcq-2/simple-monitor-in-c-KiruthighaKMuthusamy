...
// Assert statement is encapsulated inside a function 
...

{
	...
	for (int i=0;i<=3;i++)
	{
		 ...
		 assert(result == resultBattStatus[i]);
				...
				assert((int)batteryCond.breachedValue[j]  == (int)testResultarray[i].breachedValue[j]);		
				}
...
int main() 
{
    ...
    batteryCondition testResultArray[4] =  { {{"Temperature","StateofCharge","ChargeRate"},{"OK","LOW","HIGH"},{0,-0.5,0.1}},
	                                     {{"Temperature","StateofCharge","ChargeRate"},{"LOW","OK","OK"},{-1.5,0,0}}
		                            ,{{"Temperature","StateofCharge","ChargeRate"},{"HIGH","HIGH","LOW"},{5,0.5,-1}}
		                           , {{"Temperature","StateofCharge","ChargeRate"},{"OK","OK","OK"},{0,0,0}}};
